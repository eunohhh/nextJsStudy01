...
export default function Scene({ ...props }) {
    ...
    const postbox = useLoader(GLTFLoader, '/postbox.glb');
    const letter = useLoader(GLTFLoader, '/letter.glb');
    ...
    useFrame(()=>{
        ...
        if(letterRef.current){
            ...
            if(letterMoving_1){
                const targetQuaternion = new THREE.Quaternion().setFromEuler(new THREE.Euler(9.35, Math.PI, 3.14));
                letterRef.current.children[0].children[0].quaternion.slerp(targetQuaternion, 0.04);
                letterRef.current.children[0].children[0].position.lerp(new THREE.Vector3(0, 3.5, 0.3), 0.04);

                const difference = letterRef.current.children[0].children[0].quaternion.angleTo(targetQuaternion);
            ...
            if(letterMoving_2){
                letterRef.current.children[0].children[0].rotation.set(9.35, Math.PI, 3.14);
                letterRef.current.children[0].children[0].position.set(0, 3.5, 0.3);

                const targetQuaternion = new THREE.Quaternion().setFromEuler(new THREE.Euler(1.57, 3, 0));
                letterRef.current.quaternion.slerp(targetQuaternion, 0.05);
                
                // Quaternion 간의 차이 계산
                const difference = letterRef.current.quaternion.angleTo(targetQuaternion);
